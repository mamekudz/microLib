...
/** Class to compile an XML based i18x format definition into an executable formatting function.
 * @class i18xFormat
 */
class i18xFormat {
	/** Creates a new i18xFormat by parsing and compiling the given XML format definition.
	 * @param {string} _xml    The XML source of the format definition.
	 */
	constructor(_xml) {
		this.name = "unnamed";
		this.source = _xml;
	...
	/** Parses the XML format definition and builds the executable format function (this.exec).
	 * @param {string} _xml    The XML source of the format definition.
	 * @returns {void}
	 */
	#_parse = function (_xml) {
		...
		//log(_xml);
		/** Builds a comma separated string of quoted characters used for digit padding code generation.
		 * @param {number} _noOf      The total number of characters to generate.
		 * @param {string} _chr       The character to use for the leading positions.
		 * @param {number} _maxNo     The maximum number of _chr characters; remaining positions are filled with _defchr (0 = use _chr for all).
		 * @param {string} _defchr    The default character used for the positions beyond _maxNo.
		 * @returns {string}    A comma separated string of quoted characters.
		 */
		...
		/** Generates the variable definition codes for a value tag, applies the attribute wrappers (enumeration, expression, format) and appends the resulting code to the current level.
		 * @param {string} _vardefcodeName    The name of the variable definition to create (e.g. "localdate", "utcdate", "now", "value").
		 * @param {string} _code              The generated JavaScript code snippet which yields the tag value.
		 * @param {string} _vardefcode        The variable definition code assigned to the generated variable.
		 * @param {boolean} [_codeadd=true]   Whether to append the resulting code to the return codes of the current level.
		 * @returns {void}
		 */
...
/** Class to compile an i18x expression string into an executable JavaScript function.
 * @class i18xExpression
 */
export class i18xExpression {
	/** Creates a new i18xExpression by parsing and compiling the given expression source.
	 * @param {string} _exp    The i18x expression source (e.g. "round(x*100)/100").
	 */
	constructor(_exp) {
		this.source = _exp;
		this.execSource = "";
	...
	/** Parses the expression source, maps the i18x function names to JavaScript equivalents and compiles the executable function (this.exec).
	 * @param {string} _exp    The i18x expression source to compile.
	 * @returns {void}
	 */
...
export default class i18x {
	...
	/** The soft hyphen character (U+00AD) used to mark hyphenation positions.
	 * @static
	 * @type {string}
	 */
	static HYPHENCHAR = "\xAD";

	/** The horizontal writing direction of the last translation ("ltr" or "rtl").
	 * @static
	 * @type {string}
	 */
	static lastHorDir = "ltr";
	/** The vertical writing direction of the last translation ("ttb" or "btt").
	 * @static
	 * @type {string}
	 */
	static lastVerDir = "ttb";

	/** Whether the interactive translation edit mode is active.
	 * @static
	 * @type {boolean}
	 */
	static IS_I18XEDIT = false;
	/** Counter to generate unique translation edit ids.
	 * @static
	 * @type {number}
	 */
	static lastTransIdCounter = 0;
	/** The translation edit id of the last translation.
	 * @static
	 * @type {number}
	 */
	static lastTransId = 0;
	/** The native (untranslated) edit id of the last translation.
	 * @static
	 * @type {number}
	 */
	static lastTransNativeId = 0;
	/** The prefix text of the last translation (text before the i18x notation tag).
	 * @static
	 * @type {string}
	 */
	static lastTransPrefix = "";
	/** The state of the last translation (e.g. "ok", "missing").
	 * @static
	 * @type {string}
	 */
	static lastState = "ok";
	/** The context of the last translation.
	 * @static
	 * @type {string}
	 */
	static lastContext = "";
	/** The resulting text of the last translation.
	 * @static
	 * @type {string}
	 */
	static lastTransResult = "";
	/** The format type of the last translation (e.g. "text", "html").
	 * @static
	 * @type {string}
	 */
	static lastFormat = "text";
	/** The format name of the last translation.
	 * @static
	 * @type {string}
	 */
	static lastFormatName = "";
	/** The translation key of the last translation.
	 * @static
	 * @type {string}
	 */
	static lastTransKey = "";
	/** The placeholder values of the last translation.
	 * @static
	 * @type {object}
	 */
	static lastTransPlaceholders = {};
	/** The placeholder definitions of the last translation.
	 * @static
	 * @type {object}
	 */
	static lastPlaceholderDefs = {};
	/** Whether the translation key of the last translation exists in the translation table.
	 * @static
	 * @type {boolean}
	 */
	static lastTransKeyExists = false;
	/** An object with all translation edit entries, keyed by edit id.
	 * @static
	 * @type {object}
	 */
	static transEdits = {};
	/** An object with all native translation edit entries, keyed by native edit id.
	 * @static
	 * @type {object}
	 */
	static transNativeEdits = {};
	/** A map of translation keys to native edit ids.
	 * @static
	 * @type {object}
	 */
	static transEditNativeIds = {};
	/** The native edit id of the currently edited translation.
	 * @static
	 * @type {number}
	 */
	static activeEditNativeId = 0;

	/** Parse mode: translate the text.
	 * @static
	 * @type {number}
	 */
	static TRANSLATE = 0;
	/** Parse mode: do not translate the text.
	 * @static
	 * @type {number}
	 */
	static NO_TRANSLATE = 1;
	/** Parse mode: translate the text and keep the placeholders.
	 * @static
	 * @type {number}
	 */
	static TRANSLATE_WITH_PLACEHOLDERS = 2;
	/** Parse mode: translate the text and replace the placeholders with values.
	 * @static
	 * @type {number}
	 */
	static TRANSLATE_WITH_VALUES = 3;
	/** Parse mode: do not translate the text but keep the placeholders.
	 * @static
	 * @type {number}
	 */
	static NO_TRANSLATE_WITH_PLACEHOLDERS = 4;
	/** Parse mode: do not translate the text but replace the placeholders with values.
	 * @static
	 * @type {number}
	 */
	 ...
	 * @method language
	 * @static
	 * @param _lid  {string}    The localization id where to get the language code.
	 ...
	 * @method country
	 * @static
	 * @param _lid  {string}    The localization id where to get the country code.
	 ...
	 * @static
	 * @param _lid  {string}    The localization id where to get the main localization code.
	...
	/** Creates the empty runtime structures (translations, expressions, formats, templates) for a localization id, if not already present.
	 * @method createLid
	 * @static
	 * @param {string} _lid    The localization identifier to create.
	 * @returns {void}
	 */
	...
	/** Sets the current localization, resolving "auto" to the user localization and loading the localization data if necessary.
	 * @method setCurLid
	 * @static
	 * @param {string} _lid    The localization identifier to set, or "auto" to detect the best available user localization.
	 * @returns {void}
	 */
	...
	/** Parses a source file text and extracts all translatable strings (arguments of register/trans/rawTrans calls). The parser used depends on the file suffix (php, cs, vue, json, cjs, mjs, js, html).
	 * @method parse
	 * @static
	 * @param {string} _text             The source file content to parse.
	 * @param {string} [_filename=""]    The name of the source file; its suffix selects the parsing strategy.
	 * @returns {Array}    An array with all extracted translatable strings.
	 */
	...
	/** Replaces native (untranslated) strings inside a source file text; the strategy depends on the file suffix. (Work in progress, currently returns the text unchanged.)
	 * @method replaceNatives
	 * @static
	 * @param {string} _text        The source file content to process.
	 * @param {string} _filename    The name of the source file; its suffix selects the replace strategy.
	 * @param {object} _replaces    A map of native strings to their replacements.
	 ...
	 *
	 * Note: It is preferred to use the corresponding String.prototype method String.I18xTrans.
	 *
	 * @example
	 ...
	 *
	 * Note: It is preferred to use the corresponding String.prototype method String.I18xRegister.
	 *
	 * @example
	 *
	 ...
	 * @param _lid        {string}    Optional, the localization id for which the text should be registered, if not given the standard localization is used.
	 * @param _nativeText   {string}    Optional, the native (untranslated) text which should be mapped to the registered text.
	...
	/** Returns the text untranslated. This marker method excludes the given text from translation while it still gets scanned and registered by the i18x tooling.
	 * @method rawTrans
	 * @static
	 * @param {string} _text    The text which should not be translated.
	 * @param {string} _lid     The localization id (unused, kept for a signature compatible with trans).
	 * @returns {string}    The unchanged input text.
	 */
	static rawTrans(_text, _lid) {
		return _text;
	}

	/** Scans DOM text nodes for translation edit markers ({{{id}}}text) and replaces them with interactive edit elements (red dot buttons) for the translation edit mode.
	 * @method updateTransEditTextNodes
	 * @static
	 * @param {Node} [_node=null]    Optional, the DOM node to scan; if null, the whole document body is scanned.
	 * @returns {void}
	 */
	static updateTransEditTextNodes(_node = null) {
		let textNodes = [];

		/** Walks all text nodes below the given element and replaces translation edit markers with interactive edit elements.
		 * @param {Element} el    The root element whose text nodes have to be converted.
		 * @returns {void}
		 */
	...
	/** Activates the interactive translation edit mode: loads the native id index, injects the edit marker styles and observes the DOM for new text nodes to convert.
	 * @method activateTransEdit
	 * @static
	 * @returns {void}
	 */
	static activateTransEdit() {
		if (!i18x.IS_I18XEDIT) {
			...
			/** Mutation observer callback which converts translation edit markers in newly added DOM nodes.
			 * @param {MutationRecord[]} mutationList    The list of observed mutations.
			 * @param {MutationObserver} observer        The observer which triggered the callback.
			 * @returns {void}
			 */
	...
	/** Deactivates the interactive translation edit mode and persists the state.
	 * @method deActivateTransEdit
	 * @static
	 * @returns {void}
	 */
	...
	/** Returns the flag emoji of a given country code.
	 * @method flagEmojiOfCountryCode
	 * @static
	 * @param {string} _countryCode    The country code (ISO-3166), e.g. "US".
	...
	/** Returns the flag emoji of the country part of a given localization id.
	 * @method flagEmojiOfLanguageCode
	 * @static
	 * @param {string} _languageCode    The localization id (ISO-639 + ISO-3166), e.g. "en-US".
	 * @returns {string}    The flag emoji of the country.
	 */
	...
	/** Initializes the i18x system: loads the standard localization and activates the translation edit mode, if requested by local storage or url parameter.
	 * @method init
	 * @static
	 * @returns {void}
	 */
...
/** Namespace object for the interactive translation edit mode.
 * @class TRANSEDIT
 */
function TRANSEDIT() {
}

/** The window handle of the currently opened external translation editor, or null if none is open.
 * @static
 * @type {Window}
 */
TRANSEDIT.editWindow = null;
/** Opens the external translation editor for the clicked translation edit marker and synchronizes changed translations back into the page.
 * @static
 * @param {Event} _ev                    The click event of the edit marker.
 * @param {number} _editId               The translation edit id of the clicked text.
 * @param {number} [_editTooltipId=-1]   Optional, the translation edit id of the tooltip text, -1 if none.
 * @returns {void}
 */
...
/** Returns the context of an i18x text (content of the <context="..."/> tag).
 * @returns {string}    The context text, or an empty string if no context tag is present.
 */
...
/** Removes the <context="..."/> tag from an i18x text.
 * @returns {string}    The text without the context tag.
 */
...
/** Text translation of a string into a specified or current localization with decoding of HTML entities.
 * Prototype method of i18x.trans with enabled HTML entities decoding.
 * @param _placeholders   {array}    Optional, an associative array with placeholder values, the key represent the placeholder name, the corresponding values the replacements.
 *                         A value of null can be used, if no placeholders are needed.
 * @param _lid            {string}   Optional, a localization id of the destination language, a combination of iso-639 and iso-3166, e.g. "en-US".
 *                         If not given, the current language will be used.
...
/** Returns the string untranslated. This marker method excludes the string from translation while it still gets scanned and registered by the i18x tooling.
 * Prototype method of i18x.rawTrans.
 * @param _lid    {string}    Optional, the localization id (unused, kept for a signature compatible with I18xTrans).
 * @return {string}    The unchanged string.
 */
String.prototype.I18xRawTrans = function (_lid = undefined) {
	return i18x.rawTrans(this.valueOf(), _lid);
}

/** Text translation of a string into a specified or current language. Same as trans method, but the given text will not be scanned by the node module i18xe-sync.
 * @param _placeholders   {array}    Optional, an associative array with placeholder values, the key represent the placeholder name, the corresponding values the replacements.
 *                         A value of null can be used, if no placeholders are needed.
 * @param _lid            {string}   Optional, a localization id of the destination language, a combination of iso-639 and iso-3166, e.g. "en-US".
 *                         If not given, the current language will be used.
 ...
 * </format>' . i18xRegister();
 *
 * @param _lid  {string} Optional, the localization id for which the string should be registered, if not given the standard localization is used.
 * @return {string}  The i18x key of the registered string.
 */
String.prototype.I18xRegister = function (_lid = undefined) {
	return i18x.register(this.valueOf(), _lid);
}

/** Registers the string for localization. Same as I18xRegister, but the given text will not be scanned by the node module i18xe-sync.
 * @param _lid  {string} Optional, the localization id for which the string should be registered, if not given the standard localization is used.
 * @return {string}  The i18x key of the registered string.
 * @see {I18xRegister}
 */
 ...
 * ```
 * @param _lid      {string}    Optional, the localization id used to pick the text from a MLJSON (multi localization json), default "".
 * @param _doTrans  {boolean}   Optional, whether the normalized text has to be translated, default true.
...
/** Detects the i18x format notation of a string (e.g. "<xmlq/>", "{cbng/}", "[sbnq/]", "◄bpnq/►" or "JSON").
 * @return {string}    The detected notation identifier, default "<xmlg/>" if none could be detected.
 */
...
/** Converts a normalized (<xmlq/> notated) i18x string back into the given format notation.
 * @param _notation    {string}    Optional, the target notation identifier (e.g. "{cbnq/}", "{cbns/}", "{cbng/}"), default "" which means the notation is taken from the first seven characters of the string.
 * @return {string}    The re-notated i18x string, prefixed with the notation identifier.
 */
...
/** Hyphenates a single word by inserting soft hyphen characters (i18x.HYPHENCHAR) at the syllable positions of the given localization.
 * @param _lid    {string}    Optional, the localization id whose hyphenation rules have to be used, default "" which means the current localization.
 * @return {string}    The word with inserted soft hyphen characters.
 */
...
/** Hyphenates a whole text word by word; embedded XML/HTML tags are preserved. In XML/HTML texts the soft hyphens are inserted as "&shy;" entities.
 * @param _lid    {string}    Optional, the localization id whose hyphenation rules have to be used, default "" which means the current localization.
...
/** Returns the flag emoji of the string interpreted as a country code (ISO-3166), e.g. "US".
 * @return {string}    The flag emoji of the country.
 */
String.prototype.FlagEmojiOfCountryCode = function () {
	return i18x.FlagEmojiOfCountryCode(this.valueOf());
}

/** Returns the flag emoji of the country part of the string interpreted as a localization id (ISO-639 + ISO-3166), e.g. "en-US".
 * @return {string}    The flag emoji of the country.
 */
