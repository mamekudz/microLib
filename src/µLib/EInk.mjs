...
export default class EInk {
  /** Map collecting pattern usage counts (shared across all instances).
   * @type {Map}
   */
  static PATTERN_COUNTS = new Map();
  /** List of globally collected patterns (shared across all instances).
   * @type {Array}
   */
  static GLOBAL_PATTERNS = [];

  // Private state: image reference, selected display, and encoded data cache.
  #imgRef = "";
  #displayID = "EINK_NONE_DISPLAY";
  #display = null;
  #cnvEncodedData = null;

  // Private state: source image and conversion canvases with their contexts/pixel data,
  // plus the selected dither type and cut aspect.
  #imgOrg = null;
  #imgOrgCnv = null;
  #imgOrgCtx = null;
  #imgOrgData = null;
  ...
  /** Creates an e-ink image converter for the given image reference and display type.
   * @param {(string|File|Uint8Array)} _imgRef Image source: URL string, File object, or already encoded ink data (default "").
   * @param {string} _displayID Key of the target display in EInkDef.DISPLAYS (default "EINK_E6_730_sq_800x480").
   */
  ...
  /** Loads the image reference (URL, File, or encoded ink data) and prepares the
   * source and conversion canvases.
   * @returns {Promise} Resolves when the image is loaded, rejects with an error message otherwise.
   */
  Load() {
  ...
  /** Converts an RGB color to HLS color space.
   * @param {number} r Red component (0..255).
   * @param {number} g Green component (0..255).
   * @param {number} b Blue component (0..255).
   * @returns {number[]} Array [hue, lightness, saturation], each in the range 0..1.
   */
  #RGB2HLS(r, g, b) {
    r /= 255, g /= 255, b /= 255;
  ...
  /** Converts an HLS color to RGB color space.
   * @param {number} h Hue (0..1).
   * @param {number} l Lightness (0..1).
   * @param {number} s Saturation (0..1).
   * @returns {number[]} Array [r, g, b], each component in the range 0..255.
   */
  #HLS2RGB(h, l, s) {
    let r, g, b;
    if (s === 0) {
  ...
  /** Converts the loaded source image for the target display: crops/zooms to the
   * display aspect (rotating for portrait), applies saturation, black/white point,
   * and contrast corrections, and dithers to the display palette.
   * @param {Object} _opts Options: ditherType ("floyd" (default), "bayer", or nearest color), cutAspect (EInkDef.PORTRAIT/LANDSCAPE), zoom (default 1.0), cutSpotX/cutSpotY (0..1, default 0.5), cutBgrdColor (default "#ffffff"), colorSaturationAdjust, contrastAdjust, blackPointAdjust, whitePointAdjust (each default 1.0).
   */
  Convert(_opts) {
   ...
   * @param {string} hex Hex color string; falsy values yield black.
   * @returns {Object} Object with r, g, and b components (0..255).
   */
  #Hex2RGB(hex) {
  ...
  /** Encodes the converted canvas image to the version 3 ink format:
   * palette indices (masked to the display geometry) with Paeth prediction,
   * deflate-raw compression, and a 4-byte header (version, display id, dither type, orientation).
   * @returns {Promise<Uint8Array>} Encoded ink image data.
   */
  async Encode() {
  ...
  /** Returns the Paeth predictor (as used by PNG filtering) for the given neighbors.
   * @param {number} a Value of the left neighbor.
   * @param {number} b Value of the neighbor above.
   * @param {number} c Value of the neighbor above-left.
   * @returns {number} The neighbor value closest to the prediction.
   */
  ...
  /** Transforms palette indices into Paeth prediction deltas (modulo the palette size);
   * with width 0 a simple left-neighbor prediction is used.
   * @param {Uint8Array} pixels Palette indices to transform.
   * @param {number} width Row width in pixels, or 0 for linear (masked) data.
   * @param {number} numColors Number of palette colors (modulo base).
   * @returns {Uint8Array} Prediction deltas.
   */
  ...
  /** Reverses #applyPaethPrediction and restores the original palette indices.
   * @param {Uint8Array} deltas Prediction deltas to reverse.
   * @param {number} width Row width in pixels, or 0 for linear (masked) data.
   * @param {number} numColors Number of palette colors (modulo base).
   * @returns {Uint8Array} Restored palette indices.
   */
  ...
  /** Compresses data with the deflate-raw algorithm via CompressionStream.
  ...
  /** Decompresses deflate-raw data via DecompressionStream.
  ...
  /** Encodes palette indices with GIF-style LZW compression (legacy versions 0/1).
   * @param {Uint8Array} pixelsToEncode Palette indices to compress.
   * @param {Object} display Display definition providing the color palette.
   * @returns {Uint8Array} LZW compressed data.
   */
  ...
  /** Decodes encoded ink image data (format versions 0 to 3), selects the matching
   * display definition, and renders the decoded pixels onto the conversion canvas.
   * @param {Uint8Array} _data Encoded ink data; falls back to the image reference passed to the constructor.
   * @returns {Promise} Resolves when decoding has finished; returns early on invalid data.
   */
  async Decode(_data) {
  ...
  /** Decodes GIF-style LZW compressed palette indices (legacy versions 0/1).
   * @param {Uint8Array} data Encoded ink data including the header.
   * @param {number} dataPtr Offset of the compressed data within data.
   * @param {number} version Format version (0 reads the minimum code size from the header).
   * @param {number} validPixelCount Number of pixels to decode.
  ...
  /** Creates a pixel validity mask for the display geometry: a circular mask for
   * round displays, or null for square displays (all pixels valid).
   * @param {number} w Width in pixels.
   * @param {number} h Height in pixels.
   * @returns {?Uint8Array} Mask with 1 for valid and 0 for invalid pixels, or null.
   */
  ...
  /** Maps RGBA pixel data to the nearest palette color indices of the current display.
   * @param {Uint8ClampedArray} rgbaData RGBA pixel data (4 bytes per pixel).
   * @param {number} w Width in pixels.
   * @param {number} h Height in pixels.
   * @param {?Uint8Array} mask Unused geometry mask (default null).
  ...
  /** Returns the encoded ink image as a base64 data URI ("data:image/ink;base64,...").
   * @param {?Uint8Array} _imgData Already encoded ink data; if null, Encode() is called (default null).
   * @returns {Promise<string>} Data URI of the encoded image.
   */
  ...
  /** Wraps an already base64-encoded ink image string into a data URI.
   * @param {string} _imgData Base64-encoded ink image data (default null).
   * @returns {string} Data URI of the encoded image.
   */
  static GetDataURIByBase64(_imgData = null) {
    return "data:image/ink;base64," + _imgData;
  }

  /** Creates the conversion canvas if needed and resizes it to the given dimensions.
   * @param {number} w Width in pixels.
   * @param {number} h Height in pixels.
   */
  #PrepareCanvas(w, h) {
  ...
  /** Shows the converted image in an img element by assigning a PNG data URL.
   * @param {string} _toImgId DOM id of the target img element.
   */
  Show(_toImgId) {
