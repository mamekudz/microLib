...
/** Module-local counter used to enumerate the RESTX.ERROR codes (decreasing negative numbers). */
...
export default class RESTX extends XMLHttpRequest {
  /** Timestamp (milliseconds) of the last REST request, used to throttle requests. */
  static lastRestRequestTime = 0;
  /** Base URL of the webservice host. */
  static hostUrl = "/webservice";
  /** Optional global handler function called on REST transport errors. */
  static restErrorHandler = null;
  /** Registered event callbacks called when the server maintenance mode changes. */
  static onMaintanceWorkChangeEvents = {};
  /** Info objects passed to the registered maintenance mode change callbacks. */
  ...
  static ERROR = {
    ...
    /** ERROR.NEEDLESS_PROCEDURE {number} The requested procedure was needless (nothing to do). */
    ...
    /** ERROR.DLN_INVALID_OPERATION {number} At delivery notes: invalid operation. */
    DLN_INVALID_OPERATION: REST_ERROR_ENUMERATOR--,
    /** ERROR.DLN_EMPTY_MANUFACTURERNAME {number} At delivery notes: error because manufacture name is empty. */
    DLN_EMPTY_MANUFACTURERNAME: REST_ERROR_ENUMERATOR--,
    /** ERROR.DLN_PRODUCT_CONFIRMORDER_CONFLICT {number} At delivery notes: error because a product conflict occurs. */
    DLN_PRODUCT_CONFIRMORDER_CONFLICT: REST_ERROR_ENUMERATOR--,
    /** ERROR.DLN_DOUBLE_TITLE {number} At delivery notes: error because title was not unique. */
    ...
    /** ERROR.SIGNING_FAILED_SIGNATURE_MISSING {number} Signing operation failed because of a missing signature. */
    ...
    /** ERROR.MISSING_MAIL_ALLOWEDS {number} While mail send: Information mail sending not allowed by user. */
    ...
    /** ERROR.UNAUTHORIZED {number} The request was not authorized. */
    UNAUTHORIZED: REST_ERROR_ENUMERATOR--,
    /** ERROR.CONFLICT {number} The request conflicts with the current state of the target resource. */
    CONFLICT: REST_ERROR_ENUMERATOR--,
    /** ERROR.DUAL_CONTROL_VIOLATED {number} The dual control principle was violated. */
    DUAL_CONTROL_VIOLATED: REST_ERROR_ENUMERATOR--,
    /** ERROR.WRONG_USER_CANCEL {number} The user is not authorized to cancel this action. */
    WRONG_USER_CANCEL: REST_ERROR_ENUMERATOR--
  };

  /** The constructor of the RESTX object.
   * @constructor
   * @param {object} [_vueComponent=null]    Optional Vue component used to show modal error alerts.
   */
  ...
  /** Sends a RESTX request as a fire-and-forget beacon (e.g. on page unload), without waiting for a response.
   * @method sendBeacon
   * @instance
   * @param {string} _serviceName    The service name to call on server.
   * @param {object} [_input={}]     The RESTX input object.
   * @return {void}
   */
   ...
   * @param _timeout {number}         Request timeout in milliseconds, 0 for no timeout.
   * @param _limit    {boolean}       true, to throttle requests to at most one per 100ms.
   * @param _async     {boolean}      true, to send the request asynchronously.
   * @param _cache     {boolean}      true, to allow response caching.
   * @param _byRestart    {boolean}   true, if the call was triggered internally by a throttled restart.
   * @return {void}
   */
   ...
   * @param {string} _serviceName                 The service name to call on server.
   * @param {object} [_input={}]                  The RESTX input object.
   * @param {function} [_preMessageCallback=null] Optional callback invoked for every pre-message received during a long running request.
   * @return {Promise}  A promise resolved with the RESTX result object or rejected with the error result.
   *
   * @example
  ...
  /** Finishing a rest call: parses the server response, handles errors, user changes and maintenance state.
   * @method finish
   * @instance
   * @return {object}  The RESTX result object with ok, error, output and status information.
   */
  finish = function () {
    this.isRunning = false;
    let err, ret = {
  ...
  /** Restarts a rest call with the given parameters.
   * @method restart
   * @instance
   * @param {string} _action     The service name to call on server.
   * @param {object} _input      The RESTX input object.
   * @param {number} _timeout    Request timeout in milliseconds, 0 for no timeout.
   * @param {boolean} _limit     true, to throttle requests to at most one per 100ms.
   * @return {void}
   */
  ...
  /** Aborts the running rest call and marks it as aborted by the user.
   * @method abortByUser
   * @instance
   * @return {void}
   */
   ...
   * @instance
   * @param {boolean} [_withError=true]    true, to trigger the onerror handler (rejecting the promise), false to abort silently.
   * @return {void}
  ...
  /** Returns all received pre-messages of a long running request as raw (unparsed) strings.
   * @method getPreMsgsStrs
   * @instance
   * @return {array}  An array of raw pre-message strings, empty if no pre-messages were received.
   */
  ...
  /** Returns the pre-message with the given number as a parsed object:
   * ```js
   * {
   *     no:0,        // message number
   *     msg:null     // object of received json
   * }
   * ```
   * @method getPreMsgNo
   * @instance
   * @param {number} _no                  Number (index) of the requested pre-message.
   * @param {array} [_preMsgStrs=null]    Optional array of raw pre-message strings, if null the messages will be collected by getPreMsgsStrs().
   * @return {object}  An object with the message number and the parsed message, or null if not available.
   */
  ...
  /** Sets a localized error title and message for the given error code on a RESTX result object.
   * Handles HTTP status codes (positive numbers) as well as RESTX.ERROR codes (negative numbers).
   * @method setError
   * @instance
   * @param {number} _errno            The error code (HTTP status code or RESTX.ERROR code).
   * @param {object} _ret              The RESTX result object to change, a new one is created if null.
   * @param {string} [_errmsgadd=""]   Optional additional error message information.
   * @return {object}  The changed RESTX result object with ok set to false and the error filled in.
   */
  ...
  /** Sets the request headers of the rest call (content type and cache control).
   * @method setHeader
   * @instance
   * @param {string} [_contenttype="application/json"]    The content type of the request.
   * @param {boolean} [_cache=false]                       true, to allow response caching, false to send no-cache headers.
   * @return {void}
   */
  ...
  /** The RESTX instance used for server command requests, only one command request runs at a time. */
  static serverRequest = null;
  /** Executes a server command as a rest call, logs received pre-messages and calls back with the result.
   * A still running previous command request will be aborted.
   * @method serverCommandRequest
   * @instance
   * @static
   * @param {string} _command             The server command (service name) to execute.
   * @param {object} _params              The RESTX input object for the command.
   * @param {function} [_callBack=null]   Optional callback called with (success, result) when the command has finished.
   * @param {boolean} [_silent=false]     true, to suppress user interface messages.
   * @return {void}
   */
  ...
  /** Restarts a throttled rest call after the throttle delay has elapsed (used internally by call()).
   * @method #restartRest
   * @instance
   * @static
   * @param {RESTX} _this            The RESTX instance whose call will be restarted.
   * @param {string} _serviceName    The service name to call on server.
   * @param {object} _input          The RESTX input object.
   * @param {number} _timeout        Request timeout in milliseconds, 0 for no timeout.
   * @param {boolean} _limit         true, to throttle requests to at most one per 100ms.
   * @param {boolean} _async         true, to send the request asynchronously.
   * @param {boolean} _cache         true, to allow response caching.
   * @return {void}
   */
  ...
  /** Configures the base URL of the webservice host.
   * @method config
   * @instance
   * @static
   * @param {string} _hostURL    The base URL of the webservice host.
   * @return {void}
   */
  ...
  /** Calls all registered maintenance mode change callbacks with their registered info objects.
   * @method onMaintanceWorkChanged
   * @instance
   * @static
   * @return {void}
   */
  ...
  /** Registers a callback which is called when the server maintenance mode changes.
   * @method registerMaintanceWorkChanged
   * @instance
   * @static
   * @param {string} _id         Unique id of the registration.
   * @param {function} _event    Callback function called on maintenance mode changes.
  ...
  /** Unregisters a maintenance mode change callback.
   * @method unRegisterMaintanceWorkChanged
   * @instance
   * @static
  ...
  /** Initializes the RESTX class by triggering the maintenance mode change callbacks once.
   * @method init
   * @instance
   * @static
   * @return {void}
