...
/** Sprite sheet and tile atlas generation toolkit: loads image sequences,
 * extracts shadow, lighting and mask layers, removes duplicate frames,
 * bit-packs the results into texture atlases and produces the sprite/tile
 * metadata tables used by the renderer.
 * @module SpriteTools
 */
...
/** Tile info bit flags: which lighting/shadow variants a tile provides, from
 * which directions it blocks light or shadow (LSBL), which masks exist and on
 * which map layers it may be placed. Stored in the tile's TILE_INFOBITS slot.
 * @enum {number}
 */
...
/** Combined bit mask: tile blocks light/shadow from all eight directions. */
...
/** Combined bit mask: tile has at least one lighting variant. */
...
/** Combined bit mask: tile has at least one shadow or lighting variant. */
const TILE_INFOBITS_ANY_LIGT_OR_SHDW=TILE_INFOBITS_ANY_SHDW|TILE_INFOBITS_ANY_LIGT;

/** Slot indices into a tile data record (an array of TILE_DATA_LENGTH numbers):
 * info bits, atlas source coordinates (XS/YS) of the normal image, the four
 * shadow variants, the shadow mask, the nine lighting variants, the lighting
 * mask, the shading id and the atlas image numbers per layer.
 * @enum {number}
 */
...
/** Number of slots in a tile data record. */
const TILE_DATA_LENGTH=37;

/** Tile shading ids and special light emitter/direction tile numbers.
 * @enum {number}
 */
...
/** Empty tile data record (all slots zero). */
var NULLTILE=new Array(TILE_DATA_LENGTH).fill(0);
/** Tile data record used to force a redraw: flags every light/shadow variant. */
var REDRAWTILE=new Array(TILE_DATA_LENGTH).fill(0);
REDRAWTILE[TILE_INFOBITS]=TILE_INFOBITS_ANY_LIGT_OR_SHDW

/** Sprite info bit flags: which shadow/lighting images and masks a sprite
 * provides and on which render layers it may appear. Stored in the sprite's
 * SPRITE_INFOBITS slot.
 * @enum {number}
 */
...
/** Slot indices into a sprite data record (an array of SPRITE_DATA_LENGTH
 * numbers): info bits, then atlas source position (XS/YS), size (W/H) and
 * drawing offset (XO/YO) for the normal, shadow, shadow mask, lighting and
 * lighting mask images, followed by shading data and atlas image numbers.
 * @enum {number}
 */
...
/** Number of slots in a sprite data record. */
const SPRITE_DATA_LENGTH=37;

/** Layer texture ids: which atlas texture a tile or sprite image belongs to
 * (floor, things, tile/sprite shadow, tile/sprite lighting, story, actors,
 * effects, inventory, invisibles and GUI).
 * @enum {number}
 */
...
/** Short name of each layer texture, indexed by the LAYERTEXURES_* ids. */
...
/** Binary tree bin packer that grows its root node to the right or downwards
 * as needed to fit all blocks (unbounded atlas size).
 * @class SpriteToolsBitGrowingPacker
 */
SpriteToolsBitGrowingPacker=function(){};
SpriteToolsBitGrowingPacker.prototype={
	/** Packs all blocks into the growing root node; each block receives a
	 * "fit" property with its placement node (or null if it could not be placed).
	 * @param {Object[]} _blocks List of blocks with w/h properties to place.
	 * @param {boolean} _doSquare True to grow in a random direction instead of keeping the atlas square-ish.
	 */
	...
	/** Recursively searches the node tree for a free node big enough for w x h.
	 * @param {Object} root Node to start the search at.
	 * @param {number} w Required width in pixels.
	 * @param {number} h Required height in pixels.
	 * @returns {?Object} Matching free node, or null if none fits.
	 */
	...
	/** Marks a node as used and splits its remaining space into a "down" and a "right" child node.
	 * @param {Object} node Free node to occupy.
	 * @param {number} w Width in pixels of the placed block.
	 * @param {number} h Height in pixels of the placed block.
	 * @returns {Object} The occupied node (its x/y is the block position).
	 */
	...
	/** Grows the root node right or down to make room for a block that does not fit yet.
	 * @param {number} w Width in pixels of the block to place.
	 * @param {number} h Height in pixels of the block to place.
	 * @param {boolean} _doSquare True to pick the grow direction randomly instead of keeping the atlas square-ish.
	 * @returns {?Object} Placement node of the block, or null if growing was not possible.
	 */
	...
	/** Widens the root node by w pixels and places the block in the new area.
	 * @param {number} w Width in pixels to grow by (and of the block).
	 * @param {number} h Height in pixels of the block.
	 * @returns {?Object} Placement node of the block, or null on failure.
	 */
	...
	/** Heightens the root node by h pixels and places the block in the new area.
	 * @param {number} w Width in pixels of the block.
	 * @param {number} h Height in pixels to grow by (and of the block).
	 * @returns {?Object} Placement node of the block, or null on failure.
	 */
...
/** Binary tree bin packer variant that keeps the atlas square-ish and never
 * grows beyond a given maximum edge size.
 * @class SpriteToolsBitGrowingPackerStrictSquare
 */
SpriteToolsBitGrowingPackerStrictSquare=function(){};
SpriteToolsBitGrowingPackerStrictSquare.prototype={
	/** Packs all blocks into the growing root node, respecting the maximum
	 * size; aborts as soon as a block cannot be placed.
	 * @param {Object[]} _blocks List of blocks with w/h properties to place.
	 * @param {number} _maxSize Maximum allowed atlas edge length in pixels.
	 */
	...
	/** Grows the root node right or down within the maximum size to make room
	 * for a block that does not fit yet, preferring the direction that keeps
	 * the atlas square-ish.
	 * @param {number} w Width in pixels of the block to place.
	 * @param {number} h Height in pixels of the block to place.
	 * @param {number} maxSize Unused; the limit set in fit() is used instead.
	 * @returns {?Object} Placement node of the block, or null if the maximum size would be exceeded.
	 */
...
/** Main sprite/tile atlas generator: converts image sequences and legacy
 * sprite definition files into packed atlas textures plus sprite and tile
 * metadata. Progress and errors are reported through EVENT_* codes passed to
 * the caller's callback.
 * @class SpriteTools
 */
function SpriteTools(){};
ENUMERATION=1;
/** Event codes passed to progress callbacks; negative values are errors
 * (result image too big, image load failure, general error), positive values
 * report processing stages up to EVENT_READY. Available on both the class and
 * its instances.
 * @enum {number}
 */
...
/** Last reported event code (default: EVENT_NONE). */
SpriteTools.prototype.eventType=SpriteTools.EVENT_NONE;
/** Working canvas holding the current source image (default: null). */
SpriteTools.prototype.cnvImgSrc=null;
/** 2D context of the working canvas (default: null). */
SpriteTools.prototype.cnvImgSrcCtx=null;
/** ImageData object of the working canvas (default: null). */
SpriteTools.prototype.cnvImgSrcCtxImage=null;
/** Raw RGBA pixel byte array of the working canvas (default: null). */
SpriteTools.prototype.cnvImgSrcCtxImageData=null;
/** Per-pixel marker byte array used to flag already processed pixels (default: null). */
SpriteTools.prototype.memImgSrc=null;


/** Copies a canvas onto a new canvas of the given size without smoothing.
 * @param {HTMLCanvasElement} _cnv Source canvas to copy.
 * @param {number} _xSize Width in pixels of the new canvas.
 * @param {number} _ySize Height in pixels of the new canvas.
 * @returns {HTMLCanvasElement} New canvas containing the source image.
 */
...
/** Packs rectangles into a growing atlas: sorts them by largest edge, runs the
 * growing packer and writes the resulting x/y position into each rectangle.
 * @param {Object[]} _bitpackrects Rectangles with w/h properties; x/y and fit are filled in.
 * @param {Object} _options Optional packer options; doSquare selects random grow direction (default: {doSquare:true}).
 * @returns {number[]} Resulting atlas size as [width, height] in pixels.
 */
...
var l=_bitpackrects.length,i,ret=[0,0],j=0,noOfTries=1000;
	...
	/** Runs one packing pass over a copy of the rectangle list.
	 * @param {Object[]} _bitpackrects Rectangles to pack.
	 * @returns {number[]} Used atlas area as [width, height] in pixels.
	 */
...
/** Packs rectangles into a square-ish atlas limited to a maximum edge size and
 * writes the resulting x/y position into each rectangle.
 * @param {Object[]} _bitpackrects Rectangles with w/h properties; x/y and fit are filled in.
 * @param {number} _maxSize Maximum allowed atlas edge length in pixels.
 * @returns {number[]} Resulting atlas size as [width, height] in pixels.
 */
...
/** Reads a pixel from the working canvas pixel data.
 * @param {number} _x X coordinate of the pixel.
 * @param {number} _y Y coordinate of the pixel.
 * @returns {number} Pixel color packed as 32-bit ARGB-style integer.
 */
...
/** Converts a pixel of the working canvas into a shadow pixel in place: its
 * brightness becomes the alpha value while the color channels are kept.
 * @param {number} _x X coordinate of the pixel.
 * @param {number} _y Y coordinate of the pixel.
 */
...
/** Combines an original pixel with a mask pixel of the working canvas: the
 * mask brightness becomes the alpha value of the original pixel's color.
 * @param {number} _xo X coordinate of the original pixel.
 * @param {number} _yo Y coordinate of the original pixel.
 * @param {number} _xm X coordinate of the mask pixel.
 * @param {number} _ym Y coordinate of the mask pixel.
 * @returns {number} Combined pixel packed as 32-bit integer.
 */
...
/** Writes a packed pixel value into the working canvas pixel data.
 * @param {number} _x X coordinate of the pixel.
 * @param {number} _y Y coordinate of the pixel.
 * @param {number} _c Pixel color packed as 32-bit integer.
 */
...
/** Compares two equally sized boxes within the working canvas pixel by pixel.
 * @param {Object} _i First box with xs/ys/xe/ye pixel coordinates.
 * @param {Object} _j Second box with xs/ys pixel coordinates.
 * @param {number} _dubRedSen Per-channel tolerance for the comparison; 0 requires exact equality.
 * @returns {boolean} True if all pixels match within the tolerance.
 */
...
/** Checks whether a 64x64 tile area of an image is completely opaque white.
 * @param {Object} _img Image object with a cnvCtx 2D context.
 * @param {number} _xs X start of the tile in pixels.
 * @param {number} _ys Y start of the tile in pixels.
 * @returns {boolean} True if every checked pixel is opaque white.
 */
...
/** Checks whether a 64x64 tile area of an image is completely opaque black.
 * @param {Object} _img Image object with a cnvCtx 2D context.
 * @param {number} _xs X start of the tile in pixels.
 * @param {number} _ys Y start of the tile in pixels.
 * @returns {boolean} True if every checked pixel is opaque black.
 */
...
/** Checks whether a 64x64 tile area of an image is completely transparent.
 * @param {Object} _img Image object with a cnvCtx 2D context.
 * @param {number} _xs X start of the tile in pixels.
 * @param {number} _ys Y start of the tile in pixels.
 * @returns {boolean} True if every checked pixel has zero alpha.
 */
...
/** Compares equally sized areas of two canvases pixel by pixel.
 * @param {HTMLCanvasElement} _cnvA First canvas.
 * @param {HTMLCanvasElement} _cnvB Second canvas.
 * @param {number} _xsA X start of the area in the first canvas.
 * @param {number} _ysA Y start of the area in the first canvas.
 * @param {number} _xsB X start of the area in the second canvas.
 * @param {number} _ysB Y start of the area in the second canvas.
 * @param {number} _w Width of the compared area in pixels.
 * @param {number} _h Height of the compared area in pixels.
 * @param {number} _dubRedSen Per-channel tolerance for the comparison; 0 requires exact equality.
 * @returns {boolean} True if all pixels match within the tolerance.
 */
...
/** Samples the center pixel of an area and reduces it to a 16-bit sort key
 * (4 bits per channel) used to pre-sort images before duplicate comparison.
 * @param {HTMLCanvasElement} _cnv Canvas to sample from.
 * @param {number} _xs X start of the area in pixels.
 * @param {number} _ys Y start of the area in pixels.
 * @param {number} _w Width of the area in pixels.
 * @param {number} _h Height of the area in pixels.
 * @returns {number} 16-bit color sort key of the center pixel.
 */
...
/** Marks a rectangular area in the pixel marker array as already processed.
 * @param {number} _xs X start of the box (clamped to the array bounds).
 * @param {number} _ys Y start of the box (clamped to the array bounds).
 * @param {number} _xe X end of the box (inclusive).
 * @param {number} _ye Y end of the box (inclusive).
 */
...
/** Determines the bounding box of the visible content of a canvas by trimming
 * borders that match the top-left corner pixel (the background color).
 * @param {HTMLCanvasElement} _cnv Canvas to analyze.
 * @param {number} _shkSen Per-channel tolerance for treating a pixel as background; 0 requires exact equality.
 * @returns {Object} Box with xs/ys/xe/ye bounds, w/h size and xo/yo center offsets; a full 192x192 box collapses to a 1x1 dummy.
 */
...
/** Applies a grayscale mask canvas to an original image in place: the mask
 * brightness becomes the alpha channel of the original pixels.
 * @param {HTMLCanvasElement} _cnvOrg Canvas with the original image (modified in place).
 * @param {HTMLCanvasElement} _cnvMsk Canvas with the grayscale mask image.
 * @param {boolean} _isInvMsk True if the mask is inverted (bright means transparent).
 * @param {Object} _box Optional box with xs/ys/xe/ye limiting the processed area (default: whole canvas).
 */
...
/** Converts a shadow canvas into a pure alpha shadow in place: shadow darkness
 * scaled by the mask brightness becomes the alpha value, colors become black,
 * and alpha values below the trigger threshold are cleared.
 * @param {HTMLCanvasElement} _cnvShd Canvas with the shadow image (modified in place).
 * @param {HTMLCanvasElement} _cnvMsk Canvas with the grayscale mask image.
 * @param {boolean} _isInvMsk True if the mask is inverted.
 * @param {boolean} _isInvShd True if the shadow image is inverted (dark means no shadow).
 * @param {number} _shdTrig Minimum shadow intensity; weaker pixels become fully transparent.
 * @param {Object} _box Optional box with xs/ys/xe/ye limiting the processed area (default: whole canvas).
 */
...
/** Prepares a shadow canvas in place: the inverted brightness of each pixel
 * becomes its alpha value while the color channels are kept.
 * @param {HTMLCanvasElement} _cnvShd Canvas with the shadow image (modified in place).
 * @param {Object} _box Optional box with xs/ys/xe/ye limiting the processed area (default: whole canvas).
 */
...
/** Clears selected 64x64 quadrants of a 192x192 shadow image to white, given
 * as a bit mask (1: top right, 2: bottom right, 4: bottom left, 8: center).
 * @param {Object|HTMLCanvasElement} _imgCnv Image object with a cnv canvas, or the canvas itself (modified in place).
 * @param {number} _cuts Bit mask selecting the quadrants to clear.
 */
...
var img=ctx.getImageData(0,0,cnv.width,cnv.height),ps=img.data;
	/** Fills one 64x64 area with opaque white.
	 * @param {number} _xs X start of the area in pixels.
	 * @param {number} _ys Y start of the area in pixels.
	 */
...
/** Repairs a 192x192 shadow image by copying a clean center strip over
 * neighboring areas (mode 1: clear the left/bottom-left stone shadow,
 * mode 2: clear the top/top-right stone shadow).
 * @param {Object|HTMLCanvasElement} _imgCnv Image object with a cnv canvas, or the canvas itself (modified in place).
 * @param {number} _mode Copy mode (1 or 2); other values leave the image unchanged.
 */
...
/** Normalizes the alpha channel of a shadow canvas in place so its strongest
 * shadow pixel reaches full opacity; pixels below the trigger are ignored.
 * @param {HTMLCanvasElement} _cnvShd Canvas with the alpha shadow image (modified in place).
 * @param {number} _shdTrig Minimum alpha value for a pixel to take part in the normalization.
 * @param {Object} _box Optional box with xs/ys/xe/ye limiting the processed area (default: whole canvas).
 */
...
/** Converts the center 64x64 tile of a canvas into a lighting mask in place:
 * the red channel becomes the alpha value and the color becomes black.
 * @param {HTMLCanvasElement} _cnv Canvas holding the 192x192 tile sheet (modified in place).
 */
...
/** Converts a full 192x192 sprite canvas into a lighting mask in place:
 * the red channel becomes the alpha value and the color becomes black.
 * @param {HTMLCanvasElement} _cnv Canvas holding the sprite image (modified in place).
 */
...
/** Converts the center 64x64 tile of a canvas into a shadow mask in place:
 * the red channel becomes the alpha value and the color becomes white.
 * @param {HTMLCanvasElement} _cnv Canvas holding the 192x192 tile sheet (modified in place).
 */
...
/** Converts a full 192x192 sprite canvas into a shadow mask in place:
 * the red channel becomes the alpha value and the color becomes white.
 * @param {HTMLCanvasElement} _cnv Canvas holding the sprite image (modified in place).
 */
...
/** Merges a blending image into a lighting canvas in place by keeping the
 * brighter value per channel (lighten blend).
 * @param {HTMLCanvasElement} _cnvL Canvas with the lighting image (modified in place).
 * @param {HTMLCanvasElement} _cnvB Canvas with the blending image.
 * @param {Object} _box Optional box with xs/ys/xe/ye limiting the processed area (default: whole canvas).
 */
...
/** Removes transparency from a canvas in place by blending all non-opaque
 * pixels against a solid background color.
 * @param {Object|HTMLCanvasElement} _imgCnv Image object with a cnv canvas, or the canvas itself (modified in place).
 * @param {number} _r Red component of the background color (0-255).
 * @param {number} _g Green component of the background color (0-255).
 * @param {number} _b Blue component of the background color (0-255).
 * @param {?Object} _box Optional box with xs/ys/xe/ye limiting the processed area (default: null for the whole canvas).
 */
...
/** Groups a flat list of loaded images into frame records by parsing the file
 * names: the configured prefixes select original, mask or shadow, and the last
 * four digits of the pure name select the frame number.
 * @param {Object[]} _imgs Loaded images, each with a name property.
 * @param {Object} _opt Options with orgPrefix, mskPrefix and shdPrefix strings.
 * @returns {Object[]} Array indexed by frame number with {org, msk, shd} image slots.
 */
...
/** Scans the working canvas for sprites stored in the legacy DSD sheet format:
 * finds framed sprite rectangles, decodes the pixel-encoded digit signs next to
 * each frame (type and sprite number) and collects original, mask, shadow and
 * light rectangles per sprite.
 * @param {Object} _opt Options; doShd enables the processing of shadow definitions.
 * @param {Function} _callBack Progress callback invoked with this instance on scan start.
 * @returns {Object} Map of sprite number to {hasOrg, hasMsk, hasShd, hasLgt, org, msk, shd, lgt} records.
 */
...
var DSDFSignPatterns=[];
	...
	/** Reads a 3x5 pixel sign at the given position and matches it against the known digit/type patterns.
	 * @param {number} _x X position of the sign's top-left pixel.
	 * @param {number} _y Y position of the sign's top-left pixel.
	 * @param {number} _signcolor Packed color value that marks sign pixels.
	 * @returns {number} Matched DSDF_SIGN_* code, or DSDF_SIGN_NONE.
	 */
	...
	/** Reads a multi-digit decimal number encoded as consecutive pixel signs.
	 * @param {number} _noOfDSDFDigits Number of digits to read.
	 * @param {number} _x X position of the first digit sign.
	 * @param {number} _y Y position of the digit signs.
	 * @param {number} _signcolor Packed color value that marks sign pixels.
	 * @returns {number} Decoded value, or -1 if a sign is not a digit.
	 */
	...
	/** Reads a sprite info block: a type sign (original, mask, shadow, light or
	 * original-and-mask) followed by a three digit sprite number.
	 * @param {number} _x X position of the type sign.
	 * @param {number} _y Y position of the type sign.
	 * @param {number} _signcolor Packed color value that marks sign pixels.
	 * @returns {Object} Record with firstsign, valid and value fields.
	 */
	...
	/** Normalizes a rectangle in place so that xs<=xe and ys<=ye.
	 * @param {Object} _rect Rectangle with xs/ys/xe/ye fields.
	 */
	...
	/** Allocates the per-pixel marker array matching the working canvas size. */
	function InitMem(){
	var x;
	...
	/** Walks the working canvas, finds all framed sprite definitions and groups
	 * their rectangles by sprite number and type.
	 * @returns {Object} Map of sprite number to {hasOrg, hasMsk, hasShd, hasLgt, org, msk, shd, lgt} records.
	 */
...
/** Finds sprites whose original or shadow rectangles contain identical pixels
 * (within the configured tolerances) and links duplicates to the first
 * occurrence via their dub field so they are stored only once.
 * @param {Object} _sprites Map of sprite records as returned by the format scan.
 * @param {Object} _opt Options with orgDubRedSen and sdhDubRedSen comparison tolerances.
 * @param {Function} _callBack Progress callback invoked with this instance.
 */
...
/** Applies mask and shadow post-processing on the working canvas: optionally
 * replaces the base color in mask/shadow areas with white, bakes each mask
 * into the alpha channel of its original image and converts shadow areas into
 * alpha shadows.
 * @param {Object} _sprites Map of sprite records as returned by the format scan.
 * @param {Function} _callBack Progress callback invoked with this instance.
 * @param {Object} _opt Options; removeMaskBaseColor enables base color removal.
 */
...
/** Default pre-work check that never requests image preprocessing.
 * @param {Image} _img Loaded image (ignored).
 * @returns {boolean} Always false.
 */
SpriteTools.prototype.NoPreWorkImgCheck=function(_img){
	return false;
};
/** Loads a batch of images from URL strings, File objects (multi-upload input)
 * or already loaded Image objects into this.loadedImgs, optionally running a
 * per-image preprocessing callback, and reports completion or errors through
 * the callbacks.
 * @param {string[]|File[]|Image[]} _urlsOrInputOrImages Image sources to load.
 * @param {Function} _preWorkImgCheck Predicate deciding whether an image needs preprocessing.
 * @param {Function} _preWorkImgCallBack Preprocessing callback (instance, image, done) run per image; NOFUNCTION to skip.
 * @param {Function} _callBack Event callback invoked with this instance on progress and errors.
 * @param {Object} _opt Options object (unused here, passed through by callers).
 * @param {Function} _loadCallBack Callback invoked with this instance once loading finishes or fails.
 */
...
/** Finishes a scanned sprite set: reduces duplicates, bit-packs all original
 * and shadow rectangles into a result atlas, draws them, optionally scales the
 * atlas and hands the finished sprite definition (imgData plus sprite data
 * records) to the callback.
 * @param {Object} _sprites Map of sprite records as returned by the format scan.
 * @param {string} _nameSuffix Series name under which the sprites are registered.
 * @param {Object} _opts Options (reduceDub, scaleWidth/scaleHeight, scaleWithQuality, comparison tolerances).
 * @param {Function} _callBack Callback invoked with this instance; on EVENT_READY this.sprites holds the result.
 */
...
/** Loads a classic DSD-formatted sprite sheet from a URL, scans and masks the
 * contained sprites and builds a packed sprite definition from them.
 * @param {string} _url URL of the DSD-formatted sheet image.
 * @param {string} _nameSuffix Series name under which the sprites are registered.
 * @param {Function} _callBack Event callback invoked with this instance; on EVENT_READY this.result holds the sprite definition.
 * @param {Object} _opt Options; missing fields are filled with defaults (hasMsk:true, doShd:true, invMsk:false, normShd:true, shdTrig:20, reduceDub:true, tolerances 0).
 */
...
/** Loads a DSD-formatted sprite sheet from a URL, scans and masks the sprites,
 * creates a doubled-size retina working copy and produces either a fixed-grid
 * sprite strip (option createSpritesStrip) or a packed sprite definition.
 * @param {string} _url URL of the DSD-formatted sheet image.
 * @param {Function} _callBack Event callback invoked with this instance; on EVENT_READY this.result holds image data URLs and sprite data.
 * @param {Object} _opt Options; missing fields are filled with defaults (hasMsk:true, doShd:true, invMsk:false, normShd:true, shdTrig:20, reduceDub:true, tolerances 0).
 */
...
// OBSOLETE:!!!!!
/** Obsolete converter: loads an image sequence (original/mask/shadow files
 * matched by name prefixes), shrinks each frame to its bounding box, applies
 * masks and shadows, lays the frames out on a working canvas and builds a
 * packed sprite definition from them.
 * @param {string[]|File[]|Image[]} _urlsOrInputOrImages Image sources to load.
 * @param {Function} _callBack Event callback invoked with this instance; on EVENT_READY this.result holds the sprite definition.
 * @param {Object} _opt Options; missing fields are filled with defaults (prefixes ""/"A_"/"S_", doShd:true, hasMsk:true, shdTrig:20, digitCount:4, ox/oy:48, reduceDub:true).
 */
...
/** Loads a DSD-formatted sheet from a URL and rearranges the scanned sprites
 * into a fixed-grid tile image ordered by sprite number.
 * @param {string} _url URL of the DSD-formatted sheet image.
 * @param {Function} _callBack Event callback invoked with this instance; on EVENT_READY this.result holds the tile image.
 * @param {Object} _opt Options; missing fields are filled with defaults (padding:false, doShd:false, hasMsk:true, invMsk:false, normShd:true, shdTrig:20, reduceDub:false, tolerances 0).
 */
...
/** Builds GUI sprites from a list of images: groups them by series name,
 * optionally slices 9-patch style border images ("@S" name parameter),
 * bit-packs everything into a GUI atlas and fills the sprite data records.
 * @param {(string[]|HTMLInputElement|Image[])} _urlsOrInputOrImages URLs, a file input element or already loaded images.
 * @param {Function} _callBack Event callback invoked with this instance; on EVENT_READY this.result holds atlas image data and sprite tables.
 * @param {Object} [_opt] Options; missing fields are filled with defaults (orgCompare:0, shdCompare:0).
 */
...
/** Builds render sprites from 192x192 image sequences: groups the images by
 * name prefix (original, lighting, shadow, blending, masks), series and layer,
 * applies masking/shadow/lighting post-processing, removes duplicates and
 * bit-packs everything into sprite atlases with sprite data records.
 * @param {(string[]|HTMLInputElement|Image[])} _urlsOrInputOrImages URLs, a file input element or already loaded images.
 * @param {Function} _callBack Event callback invoked with this instance; on EVENT_READY this.result holds atlas image data and sprite tables.
 * @param {Object} [_opt] Options; missing fields are filled with defaults (orgCompare:0, shdCompare:0).
 */
...
/** Pre-work check for _LoadImages: returns true when a loaded image is a
 * 384x384 or 768x768 tile source with a known name prefix and therefore has
 * to be shrunk to 192x192 by TileImageAntialiasingShrink first.
 * @param {Image} _img Loaded image to check.
 * @returns {boolean} True if the image needs the antialiasing shrink pass.
 */
...
/** Shrinks a 384x384 or 768x768 tile source image to 192x192 in place using
 * smoothed per-quadrant scaling (3x3 blocks of 64x64), then reloads the image
 * from the resulting canvas and continues via the callback.
 * @param {SpriteTools} _ev Event source instance passed through to the callback.
 * @param {Image} _img Image to shrink; its src is replaced by the scaled result.
 * @param {Function} _callBack Callback invoked with _ev once the shrunk image is reloaded.
 */
...
/** Composes a crack tile in place: combines the crack image (X), the crack
 * stencil (Z), the shadow (Y) and the water/abyss background (W) source images
 * of the same series/sequence into the tile's canvas center.
 * @param {Object} _imgCnv Tile image record (layer, seriesNo, sequenceNo, cnv) whose canvas receives the result.
 * @param {Image[]} _loadedImages All loaded source images to search for the W/X/Y/Z variants.
 */
SpriteTools.prototype.CreateCrackTile=function(_imgCnv,_loadedImages){
var rDD,rD,xD,yD,zD,wD,x,y;
	/** Finds the loaded image with the given prefix matching the tile's layer,
	 * series and sequence number and returns its center 256x256 pixel data.
	 * @param {string} _prefix Single-letter file name prefix (W, X, Y or Z).
	 * @returns {Uint8ClampedArray} RGBA pixel data of the matching image area, or undefined if not found.
	 */
...
/** Builds map tiles from 192x192 image sequences: groups the images by name
 * prefix (original, lighting, shadow, pattern, blending, masks), series and
 * layer, derives shadow/lighting variants and info bits, removes duplicates
 * and bit-packs everything into tile atlases with tile data records.
 * @param {(string[]|HTMLInputElement|Image[])} _urlsOrInputOrImages URLs, a file input element or already loaded images.
 * @param {Function} _callBack Event callback invoked with this instance; on EVENT_READY this.result holds atlas image data and tile tables.
 * @param {Object} [_opt] Options; missing fields are filled with defaults (shdTrig:20, orgCompare:0, shdCompare:0).
 */
...
/** Generates the classic 32x32 shadow combination image: overlays the four
 * shadow template images (none, solid, hover, future) for every left-top/
 * top/left/self combination, keeps only non-empty results and returns the
 * strip image plus a lookup table via the callback.
 * @param {Function} _callback Called with (resultImage, infoTable); infoTable maps a combination number to its x offset or -1.
 */
...
/** Repairs the classic sprite mask sheet: every 32x32 mask cell that is
 * completely white (fully used as mask) is filled with black instead.
 * @param {Function} _callback Called with the corrected mask image.
 */
...
/** Builds the URL list of an image sequence: mask, original and optionally
 * shadow file URLs for all frames from 0 to _maxFrameNo.
 * @param {string} _baseURL Base URL of the image folder (without trailing slash).
 * @param {string} _name Base name of the image sequence.
 * @param {number} _maxFrameNo Highest frame number (inclusive).
 * @param {Object} [_opt] Options; missing fields are filled with defaults (orgPrefix:"", mskPrefix:"A_", shdPrefix:"S_", doShd:true, digitCount:4, suffix:"png").
 * @returns {string[]} URLs of all mask, original and shadow frame images.
 */
...
/** Builds the final atlas texture(s) for one layer texture id: collects all
 * tile and sprite images from the resource sheets, optionally drops unused
 * series, reduces duplicates, bit-packs the images into one or more textures
 * (respecting maximum size, raster and square/full-size constraints) and
 * updates or creates the tile/sprite data records with atlas coordinates.
 * @param {number} _layerTexture Target layer texture id (LAYERTEXURES_* constant).
 * @param {number} [_imgNoStart=0] First atlas image number to assign.
 * @param {?Object} _tiles Existing tile table to update, or null to create a new one.
 * @param {?Object} _sprites Existing sprite table to update, or null to create a new one.
 * @param {Object} _tileRscs Tile resource sheets by name (img, tiles, series).
 * @param {Object} _spriteRscs Sprite resource sheets by name (img, sprites, series).
 * @param {?Object} _unusedSeries Series to drop ({tiles:{name:count}, sprites:{name:count}}), or null to keep all.
 * @param {Object} [_opts] Options; missing fields are filled with defaults (orgCompare:0, shdCompare:0, dblReduction:true, doRetina:true, maxSize:2048, spriteTileSplit:true, tileRaster:true, spriteRaster:false, squareTexture:true, fullTextures:[128,256,1024,2048,4096], sameSize:true, layerSeparationTextures:true).
 * @returns {Object} Result with imgData (data URLs by image number), tiles {series,tiles} and sprites {series,sprites}.
 */
...
var newTileSprites=false,layerImgIndexTiles=TILE_NORM_IMGNO,layerImgIndexSprites=SPRITE_NORM_IMGNO,nop,imgSizes={},retCnv={};
	...
	/** Registers a tile slot in the unit test bookkeeping (image number -1 = not yet packed).
	 * @param {number} _no Tile number.
	 * @param {number} _subIndex Tile data slot index (TILE_*_XS constant).
	 */
	...
	/** Registers a sprite slot in the unit test bookkeeping (image number -1 = not yet packed).
	 * @param {number} _no Sprite number.
	 * @param {number} _subIndex Sprite data slot index (SPRITE_*_XS constant).
	 */
