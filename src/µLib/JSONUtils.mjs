 ...
 * @return {string}  The extended JSON text created from _o.
 */
...
JSON.extendedStringify4Editing = function (_o, _space) {
	...
	/** Stringifies an object to JSON text while replacing *function* values with their MD5 hash and
	 * collecting all *function* and *Date* values into lookup tables for later re-insertion.
	 * @param {object}  _o    A object to create a JSON from.
	 * @param {string}  _space    The JSON stringify clue.
	 * @return {string}  The JSON text with placeholders for *function* values.
	 */
 ...
 * @param {string}  _json    A JSON text string.
 * @return {boolean}  true=the JSON text contains no objects of type *function* (the security check passed).
 */
 ...
 * @param {string}  _json    The JSON or javascript object text.
 * @param {string}  [_mode="standard"]    The mode to use for parsing: ""=no parsing, returns given json string, "standard"=standard parsing, "eval"=parsing by eval method, "extended"=use extended JSON parsing.
...
/** Stringifies an object to JSON text with all double quote characters replaced by the HTML entity "&quote;".
 * @param {object}  _object    The object to stringify.
 * @return {string}  The JSON text with entity encoded double quotes.
 */
 ...
 * **NOTE:** In environments without a window object (e.g. server side) no request is made and an empty object (or "{}" for the parse modes "none"/"") is returned.
 */
...
/** Loads a JSON asynchronously from server. The callback receives null on any fails.
 * @param {string}  _url    The url of the file to load from server. if a "?" character is inside _url, the timestamp will add to url to prevent caching.
 * @param {function}  [_callBack=null]    callBack function to call after loading JSON (_jsonObject, null if failed).
 * @param {string}  [_parseMode="standard"]    Mode of JSON parsing, see JSON.modeParse.
 */
 ...
 * @param {object} _o    The object to check for cyclic references.
 * @return {boolean}    true=the object contains cyclic definitions.
 */
JSON.isCyclic = function (_o) {
	...
	/** Recursively walks the object tree and sets the detected flag when an object is found again on the current path stack.
	 * @param {object} _o    The object (subtree) to inspect.
	 * @param {string} _key    The property key of _o within its parent object.
	 */
...
/** Extended JSON parse of this string (see JSON.extendedParse).
 * @return {*} An object or an array of JSON string data.
 */
